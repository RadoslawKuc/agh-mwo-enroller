...
//		System.out.println(login);
	...
	public ResponseEntity<?> updateParticipant(@PathVariable("id") String login, @RequestBody Participant participant) {
		Participant participantsUpdate = participantService.findByLogin(login);
		if (participant == null) {
			return new ResponseEntity(HttpStatus.NOT_FOUND);
		}

		participantsUpdate.setPassword(participant.getPassword());
		participantService.put(participantsUpdate);
		return new ResponseEntity<Participant>(participant, HttpStatus.OK);
	}
